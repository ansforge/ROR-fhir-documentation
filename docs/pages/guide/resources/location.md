...
Une instance `RORLocation` est référencée par une ou plusieurs instances [RORHealthcareService]({{ '/pages/guide/resources/healthcare-service.html' | relative_url }}) correspondant aux offres réalisées sur ce lieu.
...
| --- | --- |
| Endpoint | `[BASE]/Location` — `[BASE]` correspond à la base de l'URL des API FHIR du bac-à-sable ou de la production, voir [Environnements et endpoints]({{ '/pages/guide/acces-donnees/environnements-endpoints.html' | relative_url }}) |
| Méthodes HTTP | `GET` |
...
L'ensemble des paramètres de recherche applicables à cette ressource est détaillé sur la page [Paramètres de recherche — RORLocation]({{ '/pages/guide/resources/search-params/location.html' | relative_url }}).
