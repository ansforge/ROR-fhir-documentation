...
**Récit utilisateur :**
En tant que client de l'API, je souhaite récupérer l'ensemble des lieux de réalisation de l'offre.

**Requêtes :**

```sh
GET [BASE]/Location
# récupère l'ensemble des lieux de réalisation de l'offre (actifs et inactifs)

GET [BASE]/Location?_include=Location:organization
# inclut, le cas échéant, les Organization référencées par le lieu

GET [BASE]/Location?status=active
# limite le résultat aux lieux actifs
```
...
#### 4.2 Rechercher par identifiant
**Récit utilisateur :**
En tant que client de l'API, je souhaite récupérer un lieu de réalisation de l'offre à partir de son identifiant fonctionnel, ou à partir de l'identifiant externe de synchronisation défini par le porteur d'offre pour la zone d'hébergement des lits.

**Requêtes :**

```sh
GET [BASE]/Location?identifier=XXX
# recherche le lieu dont l'identifiant fonctionnel (Location.identifier.type = 25 | JDV_J236) est XXX

GET [BASE]/Location?identifier=https://mos.esante.gouv.fr/NOS/JDV_J236-TypologieIdentifiantLieu-ROR/FHIR/JDV-J236-TypologieIdentifiantLieu-ROR|XXX
# recherche le lieu par identifiant en précisant explicitement le système de la typologie d'identifiant

GET [BASE]/Location?identifier=YYY&_include=Location:organization
# recherche le lieu par identifiant externe de synchronisation (Location.identifier.type = 26 | JDV_J236) et inclut l'Organization associée
```
...
#### 4.3 Rechercher par fonction du lieu
**Récit utilisateur :**
En tant que client de l'API, je souhaite récupérer les lieux de réalisation de l'offre correspondant à une fonction donnée (ex. hébergement, consultation), codée dans la JDV_J198-FonctionLieu-ROR issue de la TRE_R343-FonctionLieu.

**Requêtes :**

```sh
GET [BASE]/Location?type=https://mos.esante.gouv.fr/NOS/TRE_R343-FonctionLieu/FHIR/TRE-R343-FonctionLieu|001
# recherche les lieux dont la fonction est "001 | Hébergement"

GET [BASE]/Location?type=https://mos.esante.gouv.fr/NOS/TRE_R343-FonctionLieu/FHIR/TRE-R343-FonctionLieu|002&status=active
# recherche les lieux actifs dont la fonction est "002 | Consultation"
```
...
#### 4.4 Rechercher par localisation (code postal, ville, commune)
**Récit utilisateur :**
En tant que client de l'API, je souhaite récupérer les lieux de réalisation de l'offre situés dans une ville, un ensemble de villes (code postal) ou une commune identifiée par son code officiel géographique (COG).

**Requêtes :**

```sh
GET [BASE]/Location?address-postalcode=44000
# recherche les lieux dont le code postal est 44000 (Nantes)

GET [BASE]/Location?address-city=Nantes
# recherche les lieux dont la localité est Nantes

GET [BASE]/Location?commune-cog=https://mos.esante.gouv.fr/NOS/TRE_R13-CommuneOM/FHIR/TRE-R13-CommuneOM|44109
# recherche les lieux situés dans la commune de Nantes (code COG 44109)

GET [BASE]/Location?_filter=(address-postalcode eq "60000" or "76620")
# recherche les lieux situés sur les communes dont le code postal est 60000 ou 76620
```
...
#### 4.5 Rechercher par proximité géographique
**Récit utilisateur :**
En tant que client de l'API, je souhaite récupérer les lieux de réalisation de l'offre situés dans un rayon donné autour d'un point de référence (coordonnées géographiques ou code commune INSEE).

**Requêtes :**

```sh
GET [BASE]/Location?near=47.21827323906432|-1.6369631507460436|15|km
# recherche les lieux situés dans un rayon de 15 km autour du point de référence (latitude|longitude, système WGS84)

GET [BASE]/Location?near-insee-code=44162|15|km
# recherche les lieux situés dans un rayon de 15 km autour de la commune 44162 (Saint-Herblain)
```
...
#### 4.6 Rechercher par date de mise à jour
**Récit utilisateur :**
En tant que client de l'API, je souhaite récupérer les lieux de réalisation de l'offre qui ont été mis à jour depuis une date donnée, afin de synchroniser mon système d'information.

**Requêtes :**

```sh
GET [BASE]/Location?_lastUpdated=ge2024-01-01
# recherche les lieux mis à jour depuis le 1er janvier 2024

GET [BASE]/Location?_lastUpdated=ge2024-01-01T00:00:00%2B01:00&_lastUpdated=le2024-01-31T23:59:59%2B01:00
# recherche les lieux mis à jour entre le 1er et le 31 janvier 2024

GET [BASE]/Location?_lastUpdated=ge2024-01-01&_sort=_lastUpdated
# recherche les lieux mis à jour depuis le 1er janvier 2024, triés par date de mise à jour croissante
```
