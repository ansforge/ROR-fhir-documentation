...
**Récit utilisateur :**
En tant que client de l'API, je souhaite récupérer l'ensemble des organisations.

**Requêtes :**

```sh
GET [BASE]/Organization
# récupère l'ensemble des organisations (actives et inactives)

GET [BASE]/Organization?_include=Organization:partof
# récupère les entités géographiques ainsi que leurs entités juridiques de rattachement

GET [BASE]/Organization?_revinclude=HealthcareService:organization
# récupère les organisations ainsi que les offres (HealthcareService) qu'elles portent

GET [BASE]/Organization?_revinclude=PractitionerRole:organization
# récupère les organisations ainsi que les rôles professionnels (PractitionerRole) qui leur sont rattachés
```
...
#### 4.2 Rechercher par numéro FINESS (identifier)
**Récit utilisateur :**
En tant que client de l'API, je souhaite récupérer une organisation à partir de son numéro FINESS.

**Requêtes :**

```sh
GET [BASE]/Organization?identifier=750000001
# récupère l'organisation dont le numéro FINESS est 750000001

GET [BASE]/Organization?identifier=1290004324&_include=Organization:partof
# récupère l'organisation dont le numéro FINESS est 1290004324, ainsi que son entité juridique de rattachement
```
...
#### 4.3 Rechercher par raison sociale (name)
**Récit utilisateur :**
En tant que client de l'API, je souhaite rechercher une organisation à partir de sa raison sociale.

**Requêtes :**

```sh
GET [BASE]/Organization?name:contains=chu
# récupère les organisations dont la raison sociale, la dénomination ou le nom d'organisation interne contient "chu" (recherche non sensible à la casse)

GET [BASE]/Organization?name=Centre Hospitalier Universitaire de Nantes
# récupère les organisations dont le nom correspond exactement à la valeur indiquée
```
...
**Récit utilisateur :**
En tant que client de l'API, je souhaite récupérer toutes les organisations mises à jour depuis une certaine date, afin de synchroniser mon système d'information.

**Requêtes :**

```sh
GET [BASE]/Organization?_lastUpdated=ge2026-06-01
# récupère toutes les organisations mises à jour à partir du 1er juin 2026 (inclus) jusqu'à aujourd'hui

GET [BASE]/Organization?_lastUpdated=ge2026-06-01&_lastUpdated=le2026-06-30
# récupère toutes les organisations mises à jour entre le 1er et le 30 juin 2026
```
...
#### 4.5 Rechercher par type / catégorie d'établissement (type)
**Récit utilisateur :**
En tant que client de l'API, je souhaite récupérer les organisations appartenant à une catégorie d'établissement donnée, ou à un type d'organisation interne donné.

**Requêtes :**

```sh
GET [BASE]/Organization?type=https://mos.esante.gouv.fr/NOS/TRE_R66-CategorieEtablissement/FHIR/TRE-R66-CategorieEtablissement|101
# récupère les entités géographiques dont la catégorie d'établissement est 101 - Centre Hospitalier Régional (C.H.R.)

GET [BASE]/Organization?type=https://mos.esante.gouv.fr/NOS/TRE_R213-ModePriseEnCharge/FHIR/TRE-R213-ModePriseEnCharge|032
# récupère les organisations internes dont le type correspond au mode de prise en charge 032 - Consultation
```
...
#### 4.6 Rechercher par région source (_tag)
**Récit utilisateur :**
En tant que client de l'API, je souhaite récupérer toutes les organisations dont la donnée provient d'une région source donnée.

**Requêtes :**

```sh
GET [BASE]/Organization?_tag=https://mos.esante.gouv.fr/NOS/TRE_R30-RegionOM/FHIR/TRE-R30-RegionOM|11
# récupère les organisations dont la région source de la donnée est 11 - Île-de-France

GET [BASE]/Organization?_tag=https://mos.esante.gouv.fr/NOS/TRE_R30-RegionOM/FHIR/TRE-R30-RegionOM|52&type=https://mos.esante.gouv.fr/NOS/TRE_R66-CategorieEtablissement/FHIR/TRE-R66-CategorieEtablissement|101
# récupère les organisations de la région 52 - Pays de la Loire dont la catégorie d'établissement est 101 - Centre Hospitalier Régional (C.H.R.)
```
